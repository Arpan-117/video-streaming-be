...
const deleteComment = async (req, res) => {
    ...
    try {
        ...
        const deletedComment = await CommentsModel.findByIdAndDelete(commentId, { session });
        const video = await VideoModel.findById(comment.videoId);
        const foundComment = await video.comments.includes(commentId);
        if (foundComment) {
            await VideoModel.findByIdAndUpdate(comment.videoId, { $pull: { comments: commentId } }, { session });
        }
...
const updateComment = async (req, res) => {
    try {
        ...
        const updatedComment = await CommentsModel.findByIdAndUpdate(commentId, { text, date: Date.now() }, { returnOriginal: false });
        return res.status(200).json({ message: "Comment updated successfully", updatedComment });
    } catch (err) {
        return res.status(500).json({ message: "Internal server error - updateComment", error: err.message });
    }
}

export { getAllComments, postComment, deleteComment, updateComment };